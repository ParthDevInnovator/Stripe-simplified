...
const CourseDetailPage = ({params}:{params:Promise<{courseId:Id<"courses">}>}) => {
	const resolvedParamas=use(params)
  const {user,isLoaded:isUserLoaded} = useUser()
  const userData = useQuery(api.users.getUserByClerkId,{
	clerkId:user?.id??""
});
  const courseData= useQuery(api.courses.getCourseById,
	{courseId:resolvedParamas.courseId});
					...
					<div className='text-center'>
							<div className='flex flex-col items-center space-y-4'>
								...
								<p className='text-2xl font-bold mb-4'>${courseData.price.toFixed(2)}</p>
							      <PurchaseButton courseId={resolvedParamas.courseId}/>
							</div>
						</div>
				)}
			</CardContent>
